/*
Tougher Together DnD gaming Custom Script
This script has been modified to take 'hp_temp' from bar3 and apply it to bar1. Aura may still be incorrectly changed until token moves. 
        ...
        handleToken = function (obj, prev, update) {
            ...
            if (obj.get("represents") !== "" || (obj.get("represents") === "" && state.HealthColors.OneOff === true)) {
                ...
                if (type == 'Player') {
                    ...
                    var curTemp = parseInt(getAttrByName(obj.get("represents"), "hp_temp"), 10);
                    var prevTemp = prev["bar3_value"];

                    // log(`max = ${maxValue}, cur = ${curValue}, prev = ${prevValue}`);
                    // log(`cur TEMP = ${curTemp}, prev TEMP = ${prevTemp}`);

                    // Stop heals from going over max health.
                    if (curValue > maxValue) {
                        obj.set("bar1_value", maxValue)
                    }

                    // When Temp HP is added make a bar with min/max being bar total.
                    if (curTemp >= prevTemp) {
                        obj.set("bar3_value", curTemp)
                        obj.set("bar3_max", curTemp)
                    }

                    if (curValue < prevValue) {
                        // Damage was Dealt
                        var damageDealt = prevValue - curValue
                        // log(`Dealt ${damageDealt} points of damage.`)

                        if (curTemp > 0) {
                            var diff = damageDealt - curTemp

                            if (diff < 0) {
                                log(prevValue + " before changing bar1 preValue")
                                obj.set("bar1_value", Number(prevValue));
                                obj.set("bar3_value", Number(prevTemp - damageDealt));

                            } else {
                                log(Number(prevValue - diff) + " before changing bar1 preValue - diff " + diff)
                                obj.set("bar1_value", Number(prevValue - diff))
                                obj.set("bar3_value", "")
                                obj.set("bar3_max", "")
                            }
                        }
                    ...
                    // When Temp HP hits 0, erase visible bars.
                    if (curTemp <= 0) {
                        obj.set("bar3_value", "")
                        obj.set("bar3_max", "")
                    }
