...
var CombatMaster = CombatMaster || (function () {
    ...
    const styles = {
        ...
        handleStatusMarkerChange = function (obj, prev) {
            ...
            var allowRepeat = state.tougherTogetherNS.allowTriggerRepeat

            // log('Handle Status Marker Change and allowRepeat:' + allowRepeat)
            ...
            if (typeof prev.statusmarkers === 'string') {
                if (obj.get('statusmarkers') !== prev.statusmarkers) {
                    ...
                    // log("prev: " + prevstatusmarkers)
                    // log("new: " + newstatusmarkers)
                    ...
                    if (newstatusmarkers.length > 0) {
                        ...
                        if (newstatusmarkers[0] == "") {
                            newstatusmarkers.shift()
                            if (allowRepeat) {
                                state.tougherTogetherNS.allowTriggerRepeat = false;
                            }
                            if (!allowRepeat) {
                                state.tougherTogetherNS.allowTriggerRepeat = true;
                                return;
                            }
                        }
...
on('ready', function () {
    ...
    // Mhykiel Patch for copying tokens.
    // Check if the namespaced property exists, creating it if it doesn't
    if (!state.tougherTogetherNS) {
        state.tougherTogetherNS = {
            allowTriggerRepeat: true
        };
    }
