on('ready', () => {
    ...
    const displayLanguagePrompt = (selected, playerId) => {
        ...
        selected.forEach((sel) => {
            ...
            sendChat('EasySpeak', `/w "${getObj('player', playerId).get('_displayname')}" [Choose a Language](!easyspeak --lang ${languagePrompt} --message ${messagePrompt} --token ${token.id})`);
            ...
            if (state.EasySpeak.isGM) {
                ...
                sendChat('EasySpeak', `/w "gm" [GM Speak As](!easyspeak --lang ${gmLanguagePrompt} --message ${messagePrompt} --gm ${speakAsPrompt})`);
                sendChat('EasySpeak', `/w "gm" [Add Controller to Languages](!easyspeak --add)`);
                sendChat('EasySpeak', `/w "gm" [Remove Controller from Languages](!easyspeak --remove)`);
            }
        });
    };
